class Validator {
  ...
  // helper method to create common validators
  static createValidators() {
    return {
      ...
      arrayMinLength: (minLength) => (response) => {
        return (
          Array.isArray(response.data) && response.data.length >= minLength
        );
      },

      arrayMaxLength: (maxLength) => (response) => {
        return (
          Array.isArray(response.data) && response.data.length <= maxLength
        );
      },
      ...
      headerEquals: (headerName, value) => (response) => {
        return (
          response.headers &&
          response.headers[headerName.toLowerCase()] === value
        );
      },
      ...
      responseTime: (maxMs) => (response) => {
        // Note: This would need to be implemented in the ApiClient to track response times
        return response.responseTime ? response.responseTime <= maxMs : true;
      },
